...
const EVIDENCE_PROVEN_AUDIT_INSTRUCTIONS = [
  ...
  "- write every finding and strength as a telegraphic, punchy fragment; never begin with filler articles: The, This, A, or An",
  "- lead with the technical mechanism: write 'Untrusted id parameter concatenated directly into SQL string, enabling injection.' instead of 'The profile endpoint concatenates the untrusted id parameter into SQL.'",
  "- every directive starts with an imperative action verb and names its code target, such as 'Refactor query to use parameterized inputs' or 'Move app.use() call before route registration'",
  "- before returning strengths, cross-check every candidate against all findings; never praise a security, reliability, or architecture mechanism that a finding flags anywhere in the supplied context—do not praise parameterized queries if any evaluated file has a SQL-injection finding",
  "- keep every finding, directive, and strength extremely concise: one or two short sentences with exact mechanisms and code symbols, but no filler, academic phrasing, or textbook explanations",
