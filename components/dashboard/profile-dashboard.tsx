...
function hasGitHubOAuthIdentity(authUser: User | null | undefined) {
  if (getGitHubOAuthIdentity(authUser)) {
    return true;
  }
...
export function ProfileDashboard({
  ...
  const [hasPersistedGitHubConnection, setHasPersistedGitHubConnection] = useState(false);
  const [isGitHubConnectionHydrated, setIsGitHubConnectionHydrated] = useState(false);
  ...
  useEffect(() => {
    ...
    const loadPersistedGitHubConnection = async () => {
      try {
        ...
        setHasPersistedGitHubConnection(connected);
        setIsGitHubConnectionExpired(false);
        if (connected) {
    ...
    void loadPersistedGitHubConnection();
  ...
  const handleConnectGithub = useCallback(async () => {
    if (!supabase) {
      showBioToast('GitHub connection is unavailable because authentication is not configured.');
      return;
    }
    ...
    // Use the stable OAuth sign-in handshake for both first-time connections
    // and credential refreshes so Supabase owns the PKCE redirect lifecycle.
    ...
    try {
      ...
      showBioToast(
        error instanceof Error
          ? `GitHub connection could not start: ${error.message}`
          : 'GitHub connection could not start. Please try again.'
      );
    }
  ...
  const handleUnlinkGitHub = useCallback(async () => {
    ...
    try {
      ...
      if (!connectionResponse.ok) {
        throw new Error('Unable to remove the saved GitHub connection.');
      }
  ...
  useEffect(() => {
    ...
    const githubProfileChannel = supabase
      .channel(`github-profile-${user.id}`)
      .on(
        'postgres_changes',
        {
          event: 'UPDATE',
  ...
  useEffect(() => {
    if (!isGitHubProfileChecked || isGithubConnected) {
      return;
    }
  ...
  const handleTourComplete = useCallback(async () => {
    ...
    try {
      ...
      setOnboardingCompletionReady(true);
    } catch (error) {
  ...
  async function loadGitHubRepositoryTree(repository: GitHubRepository) {
    ...
    try {
      const treeUrl = new URL('/api/github/repository-tree', window.location.origin);
      treeUrl.searchParams.set('repository', repository.full_name);
      ...
      for (let index = 0; index < selectedItems.length; index += 6) {
        ...
        const batchFiles = await Promise.all(
          batch.map(async ({ repository, entry }) => {
            const blobUrl = new URL('/api/github/repository-blob', window.location.origin);
            blobUrl.searchParams.set('repository', repository.full_name);
            blobUrl.searchParams.set('sha', entry.sha);
            ...
            if (!blobResponse.ok || !blobData?.content || !blobData.encoding) {
              const requestError = new Error(
              ...
              <div className="w-full max-w-md rounded-2xl border border-gray-800 bg-gray-950 p-8 shadow-[0_0_40px_rgba(0,180,255,0.1)]">
                ...
                <div className="mt-8 flex flex-col-reverse justify-center gap-4 sm:flex-row">
                  <Button
                    ...
                    className="px-6 py-2 rounded-md transition-all font-medium text-gray-300 bg-transparent border border-gray-700 hover:text-red-400 hover:border-red-800 hover:bg-red-950/50"
                  >
